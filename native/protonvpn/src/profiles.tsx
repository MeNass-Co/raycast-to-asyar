...
export default function Command() {
  ...
  useEffect(() => { readProfiles().then(setProfiles).catch(() => setProfiles([])); }, []);
  ...
  return (
    <List isLoading={profiles === null} searchBarPlaceholder="Search profiles…" navigationTitle="Proton VPN profiles">
      {!isInstalled() ? <List.EmptyView icon={Icon.ExclamationMark} title="Proton VPN is not installed on this Mac" /> : null}
      {(profiles ?? []).map((p) => (
        <List.Item key={p} icon={icon(p)} title={p} actions={
          <ActionPanel>
            <Action title={`Connect with ${p}`} icon={Icon.Globe} onAction={async () => {
              await closeMainWindow(); await showHUD(`Proton VPN: connecting with ${p}…`);
              try { const s = await protonax("profile", p); await showHUD(s.connected ? `Proton VPN: ${s.header} · ${s.ip}` : "Proton VPN: connection failed"); }
