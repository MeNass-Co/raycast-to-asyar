...
- **Build 24 LIVE** sur MBA + MBP (md5 `fe0ae1f3…`). Mesuré sur MBA vs Raycast : logo 16..37.5 ✓, requête x=50 ✓ (« test » 31 vs 30.5), « Results » y 70.5 / x 18 / w 37.5 ✓, rangée sélectionnée 38 pt à y 89.5 (RC 88), bande x 8..741.5 ✓, titre x 53.5 ✓ Inter Medium 13 ✓ (largeur « Toggle Stage Manager » 140.5 = 140.4 calculé), accessoire bord 732.5 ✓, capsule 44 pt à 1.5 pt du bord ✓, badge deux barres 13×6 pt ✓, « Open Command » Inter SemiBold 12 ✓, keycaps anneau 18 pt.
- Reste hors de portée sans quitter le verre natif : rim 2 px dégradé (RC 1 px net), coin mesuré 20.5 (CSS 23 mais le masque natif arrondit différemment), capsule +24 L sur panneau (RC +36).
- **Build 25** : barre de défilement masquée dans la liste du launcher (`.shell-content::-webkit-scrollbar { display: none }` + `scrollbar-width: none`), défilement intact. Nassim : « 0.99:1 looks, 1.2:1 functionality (AI free) », projet déclaré terminé.
- **Builds 26–27 (05/09)** : (1) contour noir en light mode = hairline 1 px que `NSGlassEffectView` peint sur son propre bord, sombre sous NSAppearance dark ; fix = le verre déborde de 2 px (`GLASS_OVERSHOOT`, radius 25) et le masque du contentView le rogne, + `applyTheme` appelle `setPanelAppearance(light|dark)` d'après `--bg-primary` du thème (la MBP avait `theme: dark` avec Lunar Light). Vérifié : bord passe de L=33 à 162/253 (ombre système seulement). (2) capsules du pied re-mesurées **à l'intérieur du rim** Raycast : 36 pt de haut, 8 pt des bords gauche/droit, 7 pt du bas, badge 36 pt de large ; `--shell-footer-h` 43.
- **Build 27 LIVE** MBA + MBP (md5 `f6807df3…`). Vérifié en Lunar Light sur fond blanc : bord = 162→253 (ombre système seule, plus de pixel 33), étendu et compact ; capsule pied à 9.5 pt du bord droit, badge à 8.5 pt du gauche, 36 pt de haut, ≈8 pt du bas (Raycast 8/8/36/7). MBA remis en Lunar sombre après test ; MBP reste en Lunar Light avec `theme: light`.

## 2026-09-05 — Ask-X cassé + revue extensions apps (SuperWhisper, Amphetamine, f.lux, GameHub, Kofe Flow, Macs Fan Control, ProtonVPN)

- **Bug « Ask xyz »** : `extensionManager.handleDynamicCommandAction` faisait `hideWindow().then(resetLauncherState)` sans condition après le dispatch. L'agent « Ask X » (non silencieux) navigue vers `agents/AgentChatView` pendant le dispatch → la vue était pop dans la même milliseconde (log : « Navigating to view… » puis « Going back… » au même timestamp). Fix : comparer la taille de la pile avant/après ; si elle a grandi, retourner `{type:'view'}` au lieu de cacher. (Silent agents → toujours no-view.)
- **Store (index MeNass-Co) — existent, installés MBA** : `raycast.nchudleigh.superwhisper` (toggle-record, search-history, copy/paste-last-history, select-mode, open-settings ; `superwhisper://record|settings|mode?key=`), `raycast.gstvds.amphetamine` (AppleScript `tell application "Amphetamine"`), `raycast.allejo.flux` (System Events sur le menu de Flux), `raycast.samuelbelo.fancontrol` (**smctl requis, absent** → inutile).
- **Faits app (MBP)** : SuperWhisper 2.16.3, bundle `com.superduper.superwhisper`, modes = `~/Documents/superwhisper/modes/*.json` (4 : french, default, default-YAJD, dossier d'entrant scribe), `activeModeKey` dans `defaults com.superduper.superwhisper`, recordings = `~/Documents/superwhisper/recordings/<ts>/meta.json` (rawResult, llmResult…), DB `~/Library/Application Support/superwhisper/database/superwhisper.sqlite`. Menu : Toggle Recording, Transcribe File…, History…, Settings…, micro, mode, Version, Check for Updates…, Quit.
- GameHub `com.gamemac.www` : **`gamehub://launch/steam/<steamAppId>` fonctionne** (Sekiro 814380 lancé, confirmé par Nassim) ; bibliothèque = `~/Library/Application Support/com.gamemac.www/gamehub/game_container_store.json › bindings[] {game_id, game_name, platform, platform_app_id}` (RDR2 1174180, Sekiro 814380). Aussi `launch/epic|gog|local`.
- Kofe Flow (`com.rahulmfg.kofeflow`, « CoffeeFlow ») : App Intents `RunKofeFocusActionIntent(action: KofeIntentAction ∈ start|pause|skip|toggle…)` + `OpenKofeTodayIntent` ; pas d'URL scheme ; popover = Timer/Analytics/Settings + 3 boutons sans label. Voie = Shortcuts (`shortcuts run`) ou AX.
- Macs Fan Control : menu bar « Fan presets: Automatic / Full blast », « Custom… », par ventilo « Change control… » ; helper `com.crystalidea.macsfancontrol.smcwrite` root-only ; pas de CLI → AX sur le menu (fiable : 3 items).
- ProtonVPN `ch.protonvpn.mac` : seul deep link `protonvpn://refresh`, pas de CLI, pas de menu bar, profils « Stealth », « Disney plus » dans defaults. Choisir un pays sans l'app = **pas faisable proprement** → hors scope (ouvrir l'app seulement).
- Décision Nassim : « Make them native in architecture, like the other raycast extensions » → extensions Raycast-API sous `native/`, converties par rc2asyar, icône = vraie icône de l'app.
- **Natives livrées (05/09)** sous `native/` → `com.nassim.{superwhisper,amphetamine,flux,gamehub,kofeflow,macsfancontrol}` installées MBA + MBP (32 commandes `cmd_com.nassim.*` indexées), store `raycast.{nchudleigh.superwhisper,gstvds.amphetamine,allejo.flux,samuelbelo.fancontrol,mooxl.coffee}` + agent `ask-caffeinate` retirés (Nassim : Caffeinated redondant avec Amphetamine). Consentements pré-accordés dans `settings.dat › extensions.consent`.
  - Superwhisper : Select Mode (lit `~/Documents/superwhisper/modes/*.json`, actif = `defaults read com.superduper.superwhisper activeModeKey`, bascule via `superwhisper://mode?key=`), Toggle Recording (`superwhisper://record`), History (recordings/meta.json, detail + Paste/Copy), Transcribe File / Check for Updates (clic AX sur le menu bar « Transcribe File... » / « Check for Updates... »), Settings (`superwhisper://settings`).
  - Amphetamine : AppleScript `start new session [with options {duration, interval, displaySleepAllowed}]`, `end session`, `session is active`, `session time remaining` (vérifié live).
  - f.lux : AX sur le menu bar (`Disable › for an hour / until sunrise`, `Color Effects › Darkroom / Movie mode`, `Options`).
  - GameHub : liste = `game_container_store.json › bindings`, lancement `gamehub://launch/<platform>/<platform_app_id>`.
  - Kofe Flow : AX sur le popover (bouton 1 = Start/Resume, 2 = Pause, 3 = Break ; statut = static text 2), `open -b com.rahulmfg.kofeflow` pour le dashboard.
  - Macs Fan Control : AX menu bar « Automatic » / « Full blast » (l'item de barre porte la température → index 1).
  - ProtonVPN : hors scope (aucune surface scriptable ; seul `protonvpn://refresh`).

## 2026-09-05 après-midi — vérification live des natives + Ask-X (builds 28→30)

- **Ask-X, vraie cause (build 29)** : les « Ask <Extension> » sont des dynamiques **Tier 1** (`getBuiltinDynamicDispatcher('agents')`), pas Tier 2. Le fix de build 28 ne couvrait que `handleDynamicCommandAction` (Tier 2). Le chemin Tier 1 de `handleCommandAction` cachait la fenêtre sans condition juste après `navigateToView('agents/AgentChatView')`. Corrigé de la même façon (compare la pile de navigation avant/après le dispatch), 2 tests ajoutés. Vérifié : la vue chat reste ouverte sur les 2 Macs.
- **Ask Notes réponse (MBP OK, MBA KO puis OK)** : sur la MBA, `runs_history.error_message` = « You do not have permission to access the database » → TCC **Full Disk Access refusé** pour `org.asyar.app` (`auth_value 0`, « does not allow prompting ; recording denied »). La ligne avait été créée à la 1re tentative avec le csreq Developer-ID d'origine et ne matchait plus « Asyar Local Signing ». `sqlite3` en sudo (depuis t3) = « attempt to write a readonly database » (SIP protège TCC.db ; seul un process FDA — Terminal, sshd — peut écrire, et là l'UPDATE rendait `changes()=0` sans erreur). Réparé **par l'UI** : `x-apple.systempreferences:com.apple.preference.security?Privacy_AllFiles` → toggle asyar → mot de passe → « Quit & Reopen ». Nouvelle ligne rowid 159 `auth_value 2`.
- **2e cause, dans le runner Rust (build 30)** : un outil qui échoue faisait `?` **après** que le tour assistant (avec `tool_calls`) soit persisté → fil « empoisonné », DeepSeek refuse ensuite toute requête (« insufficient tool messages following tool_calls message »). `runner.rs` : l'erreur devient la sortie de l'outil (`{"error": …}`), helper `tool_output_or_error` + test. Les fils Ask Notes MBA corrompus ont été supprimés à la main (`threads/messages where agent_id='ask-notes'`).
- **Natives : cause du silence initial** = pas de ligne `shell_trusted_binaries` (copie manuelle) **puis** invite TCC Automation « asyar veut contrôler Amphetamine » jamais validée (l'osascript reste pendu, aucun HUD). `rc-install.mjs --dir <dossier-construit>` ajouté : installe un natif local par le même chemin consent + trust. **Vérifié live** : Amphetamine start/end/status (2 Macs), Macs Fan Control Automatic↔Full blast (`ActivePreset` bascule), f.lux Disable for an hour (HUD ; « Enable » non prouvé : le menu Disable montre toujours ✓ sur « for an hour »), SuperWhisper Select Mode (session précédente), GameHub Sekiro (Nassim).
- **Kofe Flow** (retour Nassim : « command failed », état = pause-café) : l'arbre AX du popover **change selon l'état** (idle/running/paused = scroll area + 3 boutons ; « Break complete » = group + 2 boutons sans nom). `lib.ts` réécrit : lecture par rôle sur `entire contents`, choix du bouton par libellé puis index ; commandes **End Break** et **Focus Status** ajoutées ; HUD si l'app est absente. Les raccourcis `session_start/…/break_end` présents sur les 2 Macs sont des **intents Jomo**, pas Kofe. Reste à vérifier sur la MBP (hors ligne depuis ~13:05 CEST).
- **Build 29 : panique au 1er hotkey** « keychain read failed: User canceled » sur les 2 Macs = invite keychain (SecurityAgent) non répondue ; « Always Allow » + mot de passe via System Events règle. Build 30 = build 29 + fix runner.
- **Sécurité** : le mot de passe sudo était dans `PLAN.md` (dépôt public) depuis `1f8de6a` → `git filter-repo --replace-text`, force-push ; clé ssh ajoutée puis retirée de `authorized_keys`. **Le fork asyar est maintenant sauvegardé : `MeNass-Co/asyar` (privé)** — le clone était shallow (push refusé « index-pack failed ») → `git fetch --unshallow` d'abord.
- Outils : `/tmp/rc-probe.sh "<requête>" [enter|view|list]` (hotkey, clic dans le champ, frappe, Entrée, auto-Allow TCC, extrait du log), `/tmp/vis` (Swift, visibilité CG du launcher), `/tmp/kofe-dump.applescript`. ⚠️ Sans clic dans le champ, les frappes partent dans Finder (un fichier de Downloads a été renommé 3× puis restauré).
- **Kofe Flow, version finale (14:30)** : le popover de barre de menus n'est **pas atteignable par AX** (0 fenêtre AX, couche CG hors écran) ; la seule surface est la **fenêtre Dashboard** (onglets Timer/Analytics/Settings), `open -g -b` la rouvre sans voler le focus. Boutons d'action anonymes, ordre du document, le large d'abord : running `[Pause]` ; paused `[Resume, Take a break, Resume]` ; break `[Pause break, Skip break]` ; break paused `[Resume break, Skip break]` ; break complete `[Start focus, …]`. AppleScript = 2–3 s par lecture (trop lent, les HUD montraient l'état d'avant, et `lines` est un mot réservé) → **helper Swift `assets/kofeax`** (`read` / `click <n>`, JSON, ≈0,05–0,2 s, attend le redraw de la phrase d'état). `rc2asyar` ajoute maintenant les **assets exécutables** à `permissionArgs['shell:spawn']` (donc trust row via rc-install). Cycle complet vérifié sur la MBP par deep link `asyar://extensions/com.nassim.kofeflow/<cmd>` : status → pause → break → end-break (=running) → pause → start → status, chaque HUD = nouvel état. Installé sur les 2 Macs.
- **Build 30 LIVE** MBA + MBP (md5 `226104a4…`) ; Ask Notes répond sur les 2 Macs (« You have 84 notes in Apple Notes. »).
- MBP joignable seulement par **Tailscale `100.71.118.42`** cet après-midi (mDNS `mbpn.local` résolu mais port 22 muet en Wi-Fi local) ; `node` absent du PATH ssh → `export PATH=/opt/homebrew/bin:$PATH`. Le clone MBP est `~/Developer/raycast-to-asyar-mba`.

## 2026-09-05 soir — GameHub et Proton VPN « dignes d'un marketplace » (retour Nassim : pas de raccourcis déguisés)

- **GameHub** (`native/gamehub`) : vue bibliothèque = `game_container_store.json › bindings` + **artwork local** (`Steam/appcache/librarycache/<appid>/header.jpg`, sinon `icons/<nom>.png` de GameHub) inliné en **data URI** (un chemin absolu part dans le cache d'icônes d'apps et ne rend pas) ; volet détail (plateforme, App ID, taille disque via `appmanifest_<id>.acf`, dossier, dates, état Running) ; actions Launch (`gamehub://launch/<platform>/<id>`), Show Install Folder, Steam Store, Open GameHub, Copy link. Running = ligne de commande `wine-engine/…/bin/wine C:\…` contenant le dossier d'installation. Vérifié à l'écran (MBA avec fixture copiée de la MBP, puis retirée).
- **Proton VPN** (`native/protonvpn`, nouveau) — l'app n'a ni CLI, ni intents exposés, ni URL de connexion (`protonvpn://` = refresh seulement). Deux surfaces réelles :
  1. **Catalogue** = `~/Library/Containers/ch.protonvpn.mac/Data/Library/Application Support/database.sqlite` (GRDB, tables `logical`/`logicalStatus`/`endpoint`, 18 215 logicals, rafraîchi par l'app). Lecture via `/usr/bin/sqlite3 -readonly -json`. Colonnes utiles : `exitCountryCode`, `state` (US), `translatedCity`, `tier` (0 = free), `feature` bitmask (1 Secure Core, 2 Tor, 4 P2P, 8 Streaming, 16 free-ish), `logicalStatus.load`. Proton écrit `UK` pour GB. Noms de pays via `Intl.DisplayNames`, drapeaux via regional indicators.
  2. **Pilotage** = fenêtre principale par AX, helper Swift **`assets/protonax`** (`status | connect <Country> [City] | cities <Country> | profiles | profile <Name> | quick | disconnect`). Pièges : le champ de recherche (SwiftUI) **ignore AXValue** et les frappes CGEvent ne filtrent pas de façon fiable ; la liste est un `AXOpaqueProviderList` **lazy** (≈10–16 lignes matérialisées) → on **écrit l'AXValue de la scrollbar** (0…1, écriture OK, relecture vide → position suivie en interne) et on page à 0,05 ; les villes sont des **frères** du pays (même profondeur), distinguées par l'icône voisine (`ic-chevron-down-filled` = pays, `ic-three-dots-vertical` = ville) ; la ligne survolée se lit `<Nom>, Connect`. `ensureWindow` détecte l'écran de login (`UsernameTextField`) → message clair. Connexion depuis la vue : **toast animé** puis HUD (fermer la vue d'abord tuerait le sidecar). Vérifié MBP : Japan/Osaka/Tokyo, Netherlands, Iceland, Zimbabwe, US Illinois/Washington, UK Manchester, profils Fastest/Stealth, disconnect, quick ; depuis le launcher : Connect view → Japan en 1 s.
...
- **Livré 09/09 11:56** : build 33 (`7e0166ef`) sur MBA et MBP. Vérifié sur MBA : « zqx » sur un hard link ne gèle plus, `Safari.app` apparaît dans Search Files, ⌘Entrée sur une app ouvre le Finder dessus. MBP : app up, snapshot reconstruit avec `/Applications` (17,5 Mo, 11:56). Astra a relu le diagnostic (dédoublonner avant le top-N, clé de ligne = fileId+path, sélection par ligne) : appliqué. Non fait : snippet `failed` sur le boundary d'AppShell (une autre exception de rendu tuerait encore le launcher) ; re-signature MBP a échoué (`errSecInternalComponent`, trousseau verrouillé en ssh) mais la signature MBA est acceptée.
- **13:20 — build 34 (`f3daad23`) sur MBA + MBP** : icônes système par ligne dans Search Files (`get_file_icon` → NSWorkspace `iconForFile:` sur le main thread, cache `icon_cache/file_<hash>.png`, clé = chemin + mtime), aperçu pour images et vidéos, glyphe générique seulement en repli. Vérifié par capture MBA (Safari.app, Notes.app, PNG en aperçu, .md en icône document). ⚠️ Deux fois de suite, `pnpm tauri build` lancé en arrière-plan juste après un commit n'a PAS recompilé le Rust (binaire daté d'avant) : toujours vérifier le mtime du binaire du bundle avant de déployer, ou builder au premier plan.
